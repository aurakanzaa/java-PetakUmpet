...
public class TicTacToeClient {
    ...
    private JFrame frame = new JFrame("Petak Umpet");
    ...
    public void play() throws Exception {
        ...
        try {
            ...
            if (response.startsWith("WELCOME")) {
                ...
                frame.setTitle("Petak Umpet - Player " + mark);
            }
            while (true) {
                response = in.readLine();
                if (response.startsWith("VALID_MOVE")) {
                    messageLabel.setText("Silahkan tunngu");
                    currentSquare.setBackground(Color.blue);      
                    currentSquare.repaint();
                } else if (response.startsWith("VALID2_MOVE")) {
                    messageLabel.setText("Silahkan tunggu");
                    currentSquare.setBackground(Color.yellow);      
                    currentSquare.repaint();
                } else if (response.startsWith("OPPONENT_MOVED")) {
                    int loc = Integer.parseInt(response.substring(15));
//                    board[loc].setBackground(Color.blue);
//                    board[loc].repaint();
                    messageLabel.setText("Silahkan tunggu");
                }else if (response.startsWith("OPPONENT2_MOVED")) {
                    int loc = Integer.parseInt(response.substring(16));
//                    board[loc].setBackground(Color.blue);
//                    board[loc].repaint();
                    messageLabel.setText("Giliran Anda");
                } else if (response.startsWith("VICTORY")) {
                    messageLabel.setText("Anda menang");
                    break;
                } else if (response.startsWith("DEFEAT")) {
                    messageLabel.setText("Anda kalah");
                    break;
                } else if (response.startsWith("TIE")) {
                    messageLabel.setText("Seri");
                    break;
    ...
    public static void main(String[] args) throws Exception {
        while (true) {
            String serverAddress = (args.length == 0) ? "192.168.43.80" : args[1];
